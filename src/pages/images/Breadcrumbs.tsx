...
const Breadcrumbs: React.FC<BreadcrumbsProps> = ({ folderPath, setFolderPath }) => (
  ...
  >
    <button
      className={`flex items-center gap-1 px-2 py-1 rounded-lg font-semibold transition-colors whitespace-nowrap
        ${folderPath.length === 0
    ...
    {folderPath.filter(folder => folder !== 'root').map((folder, idx) => (
      <React.Fragment key={idx}>
        ...
        <button
          className={`flex items-center gap-1 px-2 py-1 rounded-lg font-semibold transition-colors whitespace-nowrap truncate max-w-[150px]
