...
import { Boxes } from "@/shared/components/effects/BoxesBackground";
import { post } from "@/shared/api/apiClient";
import { type AuthenticationRequestDTO, type AuthenticationResponseDTO } from "@/shared/types/dto";
...
export default function LoginPage() {
  ...
  async function login(email: string, password: string) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const message = response.data?.message || response.statusText || "Login failed";
        console.error("Login failed:", message);
        throw new Error(message);
      }

      const fallbackMessage = response.data?.message || response.statusText || "Login failed";
      console.error("Login failed:", fallbackMessage);
      throw new Error(fallbackMessage);
    } catch (error: unknown) {
      const message = getErrorMessage(error, { fallback: "Login failed" });
      console.error("Login failed:", message);
      throw new Error(message);
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("wyvernbox-user-details", JSON.stringify(data));
      localStorage.setItem("wyvernbox-admin-remember", remember ? "true" : "false");
      ...
      const message = getErrorMessage(err, { fallback: "Login failed" });
      setError(message);
      notifyError(toastRef, "Login failed", message);
      setLoading(false);
    }
  };
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 via-blue-50 to-gray-200 overflow-hidden">
      ...
      <div className="relative z-20 shadow-input mx-auto w-full max-w-md rounded-none bg-white/90 p-4 md:rounded-xl md:p-8 dark:bg-black/80 border border-gray-100 backdrop-blur-lg">
        ...
        <form className="my-8 font-farro" style={{ fontFamily: 'Farro, sans-serif' }} onSubmit={handleSubmit}>
          <LabelInputContainer>
            ...
            <input
              ...
              placeholder="admin@wyvernbox.com"
