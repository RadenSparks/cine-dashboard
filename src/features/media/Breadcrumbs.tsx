...
const Breadcrumbs: React.FC<BreadcrumbsProps> = ({ folderPath, setFolderPath }) => (
  <nav
    className="flex items-center gap-1 text-sm mb-6 px-3 py-2 bg-blue-50 dark:bg-slate-800/50 rounded-lg shadow border border-blue-100 dark:border-slate-700 overflow-x-auto"
  ...
  >
    <button
      className={`flex items-center gap-1 px-2 py-1 rounded-lg font-semibold transition-colors whitespace-nowrap font-red-rose
        ${folderPath.length === 1 && folderPath[0] === 'root'
          ? "bg-blue-600 dark:bg-blue-700 text-white shadow"
          : "text-blue-700 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-slate-700"}
      `}
      onClick={() => setFolderPath(["root"])}
      aria-current={folderPath.length === 1 && folderPath[0] === 'root' ? "page" : undefined}
    ...
    {folderPath.filter(folder => folder !== 'root').map((folder, idx) => (
      <React.Fragment key={idx}>
        <span className="mx-1 text-blue-300 dark:text-slate-600 select-none shrink-0">/</span>
        <button
          ...
          style={{ fontFamily: 'Red Rose, sans-serif' }}
        >
          <svg className="w-4 h-4 mr-1 text-blue-400 dark:text-blue-300 shrink-0" fill="currentColor" viewBox="0 0 20 20">
