...
const GallerySlider = ({ images, renderAction }: GallerySliderProps) => {
  ...
  return (
    ...
    >
      {/* Main image preview */}
      <div className="relative w-full max-w-md h-56 flex items-center justify-center bg-gradient-to-br from-blue-50 to-white rounded-xl shadow-lg border border-blue-100 group transition-all duration-300">
        <img
          src={getUrl(images[current])}
          alt={`Ảnh ${current + 1}`}
          className="w-full h-full object-cover rounded-xl border border-gray-200 transition-all duration-300 shadow"
          draggable={false}
